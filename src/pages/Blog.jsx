          ...
          {loading
            ...
            : filteredProjek.map((item, index) => (
                <div key={index} className="md:flex w-full">
                  <div
                    className="image2 bg-center rounded-l-3xl bg-cover w-full md:h-80 h-56"
                    style={{ backgroundImage: `url(${item.foto_pj})` }}
                  ></div>
                  <div className="w-full px-11 py-7 justify-between rounded-r-3xl h-80 flex flex-col gap-2 bg-[#f8f8f8]">
                    ...
                    <Link className="bg-[#1f1f1f] text-white rounded-xl py-2 px-4 w-fit">Lihat detail <span class="arrow">&gt;</span></Link>
                  </div>
                </div>
              ))}
        </section>
