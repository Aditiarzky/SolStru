...
const OrderManagement = () => {
  ...
  const sendEmail = (email, nama) => {
    ...
    emailjs
      ...
      .then(
        (response) => {
          alert("Berhasil mengirim email penerimaan kepada pemesan!")
          ...
          <div>
            {visibleOrders.map((p) => (
              <li key={p.psid} className="mb-1">
                <div className="py-5 px-8 bg-white rounded-3xl flex md:justify-between md:flex-row gap-2 flex-col md:items-center">
                  <div>
                  ...
                  <div className="space-x-2">
                    {p.status_pesan === "pending" && (
                      ...
                      >
                        {approvingId === p.psid ? "Approving..." : "Approve"}
                      </button>
                    )}
                    <button
